...
const sc_exprandrange = require("../util/sc_exprandrange");
const sc_randrange = require("../util/sc_randrange");
const $i2n = `
...
dspProcess["+"]["ak"] = function(inNumSamples) {
  ...
  const aIn = this.inputs[0];
  const b = this._b;
  const next_b = this.inputs[1][0];
  const b_slope = (next_b - this._b) * this._slopeFactor;
...
dspProcess["+"]["ka"] = function(inNumSamples) {
  ...
  const a = this._a;
  const bIn = this.inputs[1];
  const next_a = this.inputs[0][0];
  const a_slope = (next_a - this._a) * this._slopeFactor;
