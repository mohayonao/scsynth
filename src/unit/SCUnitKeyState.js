...
class SCUnitKeyState extends SCUnit {
  initialize(rate) {
...
dspProcess["next"] = function () {
  let keyState = this.inputs[0][0];
  let minval = this.inputs[1][0];
  let maxval = this.inputs[2][0];
  ...
  let y0 = (keyState | 0) === this._pointVal[0] ? maxval : minval;
